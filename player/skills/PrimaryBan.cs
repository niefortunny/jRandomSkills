    ...
    public class PrimaryBan : ISkill
    {
        ...
        public static void WeaponEquip(EventItemEquip @event)
        {
            ...
            if (player == null || !player.IsValid)
                return;
            if (!bannedPlayers.ContainsKey(player.SteamID) || !disabledWeapons.Contains(weapon))
                return;
        ...
        public static void OnTick()
        {
            ...
            foreach (var player in Utilities.GetPlayers())
            {
                ...
                ConcurrentBag<(string, string)> menuItems = new(
                    enemies.Select(e => (e.PlayerName, e.Index.ToString()))
                );
        ...
        public static void TypeSkill(CCSPlayerController player, string[] commands)
        {
            ...
            var enemy = Utilities
                .GetPlayers()
                .FirstOrDefault(p => p.Team != player.Team && p.Index.ToString() == enemyId);
            ...
            player.PrintToChat(
                $" {ChatColors.Green}"
                    + player.GetTranslation("primaryban_player_info", enemy.PlayerName)
            );
            enemy.PrintToChat(
                $" {ChatColors.Red}" + player.GetTranslation("primaryban_enemy_info")
            );
        }
        ...
        private static void CheckWeapon(CCSPlayerController player)
        {
            ...
            if (activeWeapon == null || !activeWeapon.IsValid)
                return;
            if (
                activeWeapon.DesignerName == null
                || string.IsNullOrEmpty(activeWeapon.DesignerName)
            )
                return;

            if (
                !bannedPlayers.ContainsKey(player.SteamID)
                || !disabledWeapons.Contains(activeWeapon.DesignerName?.Replace("weapon_", ""))
            )
                return;
            player.ExecuteClientCommand("slot3");
        }
