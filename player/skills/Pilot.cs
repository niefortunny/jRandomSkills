...
namespace src.player.skills
{
    public class Pilot : ISkill
    {
        ...
        {
            PlayerPilotInfo.TryAdd(
                player.SteamID,
                new Pilot_PlayerInfo
                {
            ...
            if (PlayerPilotInfo.TryGetValue(player.SteamID, out var pilotInfo))
            {
                pilotInfo.Fuel = Math.Min(
                    Math.Max(
                        ...
                        pilotInfo.Fuel
                            - (
                                buttons.HasFlag(PlayerButtons.Use)
                ...
                if (buttons.HasFlag(PlayerButtons.Use))
                    if (
                        ...
                        && !player.PlayerPawn.Value.IsDefusing
                    )
        ...
        private static void UpdateHUD(CCSPlayerController player, Pilot_PlayerInfo pilotInfo)
        {
            ...
            var maximumFuel = SkillsInfo.GetValue<float>(skillName, "maximumFuel");
            if (
                pilotInfo.Fuel == maximumFuel
                && playerInfo.SkillDescriptionHudExpired >= DateTime.Now
            )
            {
            ...
            playerInfo.PrintHTML =
                $"{player.GetTranslation("pilot_hud_info")}: <font color='{fuelColor}'>{(pilotInfo.Fuel / maximumFuel) * 100:F0}%</font>";
        }

        private static string GetFuelColor(float fuelPercentage)
        {
            var maximumFuel = SkillsInfo.GetValue<float>(skillName, "maximumFuel");
            if (fuelPercentage > (maximumFuel / 2f))
                return "#00FF00";
            if (fuelPercentage > (maximumFuel / 4f))
                return "#FFFF00";
            return "#FF0000";
        }
        ...
        private static void ApplyPilotEffect(CCSPlayerController player)
        {
            ...
            Vector jetpackVelocity = new(eye_vector.X * 5.0f, eye_vector.Y * 5.0f, 0.80f * 15.0f);
        ...
        public class SkillConfig(
            ...
            float maximumFuel = 150f,
            float fuelConsumption = .64f,
            float refuelling = .1f
        )
