...
namespace src.player.skills
{
    public class MoneySwap : ISkill
    {
        ...
        public static void OnTick()
        {
            ...
            foreach (var player in Utilities.GetPlayers())
            {
                ...
                ConcurrentBag<(string, string)> menuItems = new(
                    enemies.Select(e =>
                        (
                            $"{e.PlayerName} : {(e.InGameMoneyServices == null ? 0 : e.InGameMoneyServices.Account)}$",
                            e.Index.ToString()
                        )
                    )
                );
        ...
        public static void TypeSkill(CCSPlayerController player, string[] commands)
        {
            ...
            player.PrintToChat(
                $" {ChatColors.Green}"
                    + player.GetTranslation("moneyswap_player_info", enemy.PlayerName)
            );
        ...
        {
            if (player == null || !player.IsValid || enemy == null || !enemy.IsValid)
                return;

            var playerMoneyServices = player.InGameMoneyServices;
            var enemyMoneyServices = enemy.InGameMoneyServices;
            if (playerMoneyServices == null || enemyMoneyServices == null)
                return;
