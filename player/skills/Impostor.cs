    ...
    public class Impostor : ISkill
    {
        ...
        public static void EnableSkill(CCSPlayerController player)
        {
            ...
            if (
                ...
                && player.PlayerPawn.Value.CBodyComponent != null
                && player.PlayerPawn.Value.CBodyComponent.SceneNode != null
            )
                originalModels.TryAdd(
                    player.SteamID,
                    player
                        .PlayerPawn.Value.CBodyComponent.SceneNode.GetSkeletonInstance()
        ...
        {
            var model =
                originalModels.TryGetValue(player.SteamID, out var originalModel)
                && !string.IsNullOrEmpty(originalModel)
                    ? originalModel
                : player.Team == CsTeam.Terrorist ? defaultTModel
                : defaultCTModel;
            SetPlayerModel(player, model);
        ...
        {
            CCSPlayerController[] models =
            [
                .. Utilities
                    .GetPlayers()
                    .FindAll(p => p.IsValid && p.PawnIsAlive && p.Team != player.Team),
            ];
            if (models != null && models.Length > 0)
            {
                ...
                if (
                    model != null
                    && model.IsValid
                    && model.PlayerPawn.Value != null
                    && model.PlayerPawn.Value.IsValid
                    && model.PlayerPawn.Value.CBodyComponent != null
                    && model.PlayerPawn.Value.CBodyComponent.SceneNode != null
                )
                {
