    ...
    public class Gambler : ISkill
    {
        ...
        public static void TypeSkill(CCSPlayerController player, string[] commands)
        {
            ...
            var skill = SkillData.Skills.FirstOrDefault(s =>
                player.GetSkillName(s.Skill).Equals(commands[0], StringComparison.OrdinalIgnoreCase)
                || s.Skill.ToString().Equals(commands[0], StringComparison.OrdinalIgnoreCase)
            );
            if (skill == null)
            {
                player.PrintToChat(
                    $" {ChatColors.Red}" + player.GetTranslation("skill_not_found_setskill")
                );
                return;
            }
            ...
            Instance.AddTimer(
                ...
                () =>
                {
                    ...
                    if (
                        SkillsInfo.GetValue<bool>(skill.Skill, "disableOnFreezeTime")
                            ...
                            () =>
                            {
                                if (
                                    Instance.SkillPlayer.FirstOrDefault(p =>
                                        p.SteamID == player.SteamID && p.Skill == skill.Skill
                                    ) == null
                                )
                                    return;
        ...
        private static void TakeMoney(CCSPlayerController player)
        {
            ...
            player.InGameMoneyServices.Account = Math.Max(
                0,
                account - SkillsInfo.GetValue<int>(skillName, "refreshPrice")
            );
        ...
        public static void EnableSkill(CCSPlayerController player)
        {
            ...
            [
                (player.GetSkillName(firstSkill.Skill), firstSkill.Skill.ToString()),
            ...
            SkillUtils.CreateMenu(
                ...
                menuItems,
                (
                    player.GetTranslation(
                        "gambler_more",
                        SkillsInfo.GetValue<int>(skillName, "refreshPrice")
                    ),
                    skillName.ToString()
                )
            );
        ...
        private static List<jSkill_SkillInfo> GetSkills(CCSPlayerController player)
        {
            ...
            {
                SkillsInfo.DefaultSkillInfo[] skillsNeedsTeammates = SkillsInfo
                    .LoadedConfig.Where(s => s.NeedsTeammates)
                    .ToArray();
                skillList.RemoveAll(s =>
            ...
            if (player.Team == CsTeam.Terrorist)
                skillList.RemoveAll(s =>
                    Event.counterterroristSkills.Any(s2 => s2.Name == s.Skill.ToString())
                );
            else
                skillList.RemoveAll(s =>
                    Event.terroristSkills.Any(s2 => s2.Name == s.Skill.ToString())
                );
