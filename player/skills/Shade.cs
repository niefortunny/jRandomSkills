...
namespace src.player.skills
{
    public class Shade : ISkill
    {
        ...
        private static unsafe bool CheckTeleport(
        ...
        private static void TeleportAttackerBehindVictim(
        ...
        )
        {
            ...
            Vector victimEyePos = new(
                victimPawn.AbsOrigin.X,
                victimPawn.AbsOrigin.Y,
                victimPawn.AbsOrigin.Z + victimPawn.ViewOffset.Z
            );
            int[] angles = [0, 90, -90];
            ...
            foreach (int extraAngle in angles)
            {
                ...
                Vector behindPosition =
                    victimEyePos
                    - SkillUtils.GetForwardVector(newAngle)
                        * SkillsInfo.GetValue<float>(skillName, "teleportDistance");
                if (!CheckTeleport(victim, victimEyePos, behindPosition))
                    continue;
            ...
            if (!teleported)
                noSpace.AddOrUpdate(
                    attacker,
                    Server.TickCount + (64 * 2),
                    (k, v) => Server.TickCount + (64 * 2)
                );
        }
