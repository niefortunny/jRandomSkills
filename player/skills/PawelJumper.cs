...
namespace src.player.skills
{
    public class PawelJumper : ISkill
    {
        ...
        public static void EnableSkill(CCSPlayerController player)
        {
            ...
            var skillConfig = SkillsInfo.LoadedConfig.FirstOrDefault(s =>
                s.Name == skillName.ToString()
            );
            if (skillConfig == null)
                return;

            float extraJumps = (float)
                Instance.Random.Next(
                    SkillsInfo.GetValue<int>(skillName, "extraJumpsMin"),
                    SkillsInfo.GetValue<int>(skillName, "extraJumpsMax") + 1
                );
            playerInfo.SkillChance = extraJumps;
            SkillUtils.PrintToChat(
                player,
                $"{ChatColors.DarkRed}{player.GetSkillName(skillName)}{ChatColors.Lime}: {player.GetSkillDescription(skillName, extraJumps)}",
                false
            );
        }
        ...
        private static void GiveAdditionalJump(CCSPlayerController player)
        {
            ...
            else if ((flags & PlayerFlags.FL_ONGROUND) != 0)
            {
                ...
                (LB[player.Slot] & PlayerButtons.Jump) == 0
                && (buttons & PlayerButtons.Jump) != 0
                && J[player.Slot] < playerInfo.SkillChance
            )
            {
                J[player.Slot]++;
