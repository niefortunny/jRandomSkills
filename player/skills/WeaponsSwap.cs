...
namespace src.player.skills
{
    public class WeaponsSwap : ISkill
    {
            ...
            if (cooldown == 0)
            {
                if (showInfo)
                    playerInfo.PrintHTML =
                        ...
                        : skillInfo != null && !skillInfo.HaveWeapon
                            ? $"<font color='#FF0000'>{player.GetTranslation("weaponsswap_hud_info2")}</font>"
                        : null;
                else
                    SkillUtils.ResetPrintHTML(player);
                return;
        ...
        public static void UseSkill(CCSPlayerController player)
        {
            ...
            if (SkillPlayerInfo.TryGetValue(player.SteamID, out var skillInfo))
            {
                ...
                if (skillInfo.CanUse)
                {
                    ...
                    if (
                        playerWeapon == null
                        || playerWeapon.FirstOrDefault(w => weapons.Contains(w)) == null
                    )
                    {
                    ...
                    Server.NextFrame(() =>
                    {
                        ...
                        GiveWeapons(
                            enemy,
                            playerWeapon,
                            (enemyWeapon != null && enemyWeapon.Contains("weapon_c4"))
                        );
                    });
                }
                else
        ...
        private static string[]? GetWeapons(CCSPlayerController player)
        {
            ...
            if (pawn == null || !pawn.IsValid || player.LifeState != (byte)LifeState_t.LIFE_ALIVE)
                return null;
        ...
        {
            CCSPlayerController[] enemies =
            [
                .. Utilities.GetPlayers().FindAll(e => e.Team != player.Team && e.PawnIsAlive),
            ];
            if (enemies.Length == 0)
                return null;
        ...
        private static void RemoveC4(CCSPlayerController player)
        {
            ...
            foreach (var item in pawn.WeaponServices.MyWeapons)
                if (
                    ...
                    && item.Value != null
                    && item.Value.IsValid
                    && item.Value.DesignerName == "weapon_c4"
                )
