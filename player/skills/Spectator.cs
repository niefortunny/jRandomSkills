...
namespace src.player.skills
{
    public class Spectator : ISkill
    {
        ...
        private static readonly ConcurrentDictionary<
            ulong,
            (uint, CDynamicProp, CCSPlayerPawn)
        > cameras = [];
        ...
        public static void WeaponPickup(EventItemPickup @event)
        {
            ...
            if (pawn == null || !pawn.IsValid || pawn.CameraServices == null)
                return;
            if (
                cameras.TryGetValue(player.SteamID, out var cameraInfo)
                && cameraInfo.Item1 != pawn.CameraServices.ViewEntity.Raw
            )
        ...
        private static void ChangeCamera(CCSPlayerController player, bool forceToDefault = false)
        {
            ...
            bool defaultCam =
                forceToDefault || (pawn.CameraServices.ViewEntity.Raw != orginalCameraRaw);
        ...
        private static uint CreateCamera(CCSPlayerController player)
        {
            ...
            var enemies = Utilities
                .GetPlayers()
                .Where(p => p.PawnIsAlive && p.Team != player.Team)
                .ToList();
            if (enemies.Count == 0)
                return 0;
            ...
            var pos =
                ...
                - SkillUtils.GetForwardVector(pawn.EyeAngles)
                    * SkillsInfo.GetValue<float>(skillName, "distance");
            ...
            if (cameras.TryGetValue(player.SteamID, out var cameraInfo))
                cameras.AddOrUpdate(
                    player.SteamID,
                    (cameraInfo.Item1, camera, pawn),
                    (k, v) => (cameraInfo.Item1, camera, pawn)
                );
            else
                cameras.AddOrUpdate(
                    player.SteamID,
                    (pawn.CameraServices.ViewEntity.Raw, camera, pawn),
                    (k, v) => (pawn.CameraServices.ViewEntity.Raw, camera, pawn)
                );
