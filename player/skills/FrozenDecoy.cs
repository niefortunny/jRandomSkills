    ...
    public class FrozenDecoy : ISkill
    {
        ...
        public static void DecoyDetonate(EventDecoyDetonate @event)
        {
            ...
            if (playerInfo?.Skill != skillName)
                return;
            foreach (
                var decoy in decoys.Keys.Where(v =>
        ...
        public static void OnTick()
        {
            ...
            foreach (
                var player in Utilities
                    .GetPlayers()
                    .Where(p => p.Team == CsTeam.Terrorist || p.Team == CsTeam.CounterTerrorist)
            )
            {
                ...
                if (distance <= decoyRadius)
                {
                    ...
                    pawn.VelocityModifier = (float)
                        Math.Pow(
                            modifier,
                            SkillsInfo.GetValue<int>(skillName, "slownessMultiplier")
                        );
                }
            }
        }
        ...
        public class SkillConfig(
            ...
            float triggerRadius = 180,
            int slownessMultiplier = 5
        )
