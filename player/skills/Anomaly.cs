            ...
            foreach (var player in Utilities.GetPlayers())
            {
                ...
                if (playerInfo?.Skill == skillName)
                    if (SkillPlayerInfo.TryGetValue(player.SteamID, out var skillInfo))
                    {
                        ...
                        if (Server.TickCount % tickRate != 0)
                            return;
                        var pawn = player.PlayerPawn.Value;
                        if (pawn != null && pawn.IsValid && pawn.AbsOrigin != null)
                        {
                            if (skillInfo.LastRotations == null || skillInfo.LastPositions == null)
                                continue;
                            skillInfo.LastPositions.Enqueue(
                                new Vector(pawn.AbsOrigin.X, pawn.AbsOrigin.Y, pawn.AbsOrigin.Z)
                            );
                            skillInfo.LastRotations.Enqueue(
                                new QAngle(pawn.EyeAngles.X, pawn.EyeAngles.Y, pawn.EyeAngles.Z)
                            );
                            if (
                                skillInfo.LastRotations.Count
                                > SkillsInfo.GetValue<int>(skillName, "secondsInBack")
                            )
                            {
        ...
        public static void UseSkill(CCSPlayerController player)
        {
            ...
            if (SkillPlayerInfo.TryGetValue(player.SteamID, out var skillInfo))
            {
                ...
                if (skillInfo.CanUse)
                {
                    ...
                    if (
                        skillInfo.LastRotations == null
                        || skillInfo.LastRotations.IsEmpty
                        || skillInfo.LastPositions == null
                        || skillInfo.LastPositions.IsEmpty
                    )
                        return;
