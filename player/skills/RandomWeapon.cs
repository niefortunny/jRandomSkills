        ...
        private static void RemoveAndGiveWeapon(CCSPlayerController player)
        {
            ...
            foreach (var item in pawn.WeaponServices.MyWeapons)
                if (
                    ...
                    && item.Value != null
                    && item.Value.IsValid
                    && !string.IsNullOrEmpty(item.Value.DesignerName)
                )
            ...
            ConcurrentBag<string> weaponList =
            [
                .. pistols.Concat(rifles).Where(w => !playerWeapons.Contains(w)),
            ];
            ...
            if (!string.IsNullOrEmpty(weaponToRemove))
            {
                foreach (var item in pawn.WeaponServices.MyWeapons)
                {
                    if (
                        ...
                        && item.Value != null
                        && item.Value.IsValid
                        && item.Value.DesignerName == weaponToRemove
                    )
