    ...
    public class PsychicDefusing : ISkill
    {
        ...
        public static void OnTick()
        {
            ...
            foreach (var skillInfo in SkillPlayerInfo)
            {
                ...
                if (playerController == null || !pawn.Controller.IsValid)
                    return;
                ...
                if (
                    pawn.AbsOrigin == null
                    || SkillUtils.GetDistance(pawn.AbsOrigin, bombLocation)
                        > SkillsInfo.GetValue<float>(skillName, "maxDefusingRange")
                )
                {
                    info.Defusing = false;
        ...
        private static void UpdateHUD(CCSPlayerController player, PlayerSkillInfo skillInfo)
        {
            ...
            if (cooldown == 0)
                ...
                playerInfo.PrintHTML =
                    $"{player.GetTranslation("psychicdefusing_hud_info", $"<font color='#00d5ff'>{cooldown}</font>")}";
        }
        ...
        public class SkillConfig(
            ...
            float maxDefusingRange = 80f,
            float defusingTime = 10f
        )
