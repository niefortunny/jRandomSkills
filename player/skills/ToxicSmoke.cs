    ...
    public class ToxicSmoke : ISkill
    {
        ...
        public static void OnEntitySpawned(CEntityInstance entity)
        {
            ...
            if (
                grenade == null
                || !grenade.IsValid
                || grenade.OwnerEntity == null
                || !grenade.OwnerEntity.IsValid
                || grenade.OwnerEntity.Value == null
                || !grenade.OwnerEntity.Value.IsValid
            )
                return;
        ...
        public static void SmokegrenadeExpired(EventSmokegrenadeExpired @event)
        {
            ...
            if (playerInfo?.Skill != skillName)
                return;
            foreach (
                var smoke in smokes.Keys.Where(v =>
                    v.X == @event.X && v.Y == @event.Y && v.Z == @event.Z
                )
            )
                ...
                if (Server.TickCount % 17 == 0)
                    if (
                        ...
                        if (
                            ...
                            if (
                                SkillUtils.GetDistance(smokePos, player.PlayerPawn.Value.AbsOrigin)
                                <= SkillsInfo.GetValue<float>(skillName, "smokeRadius")
                            )
                                SkillUtils.TakeHealth(
                                    player,
                                    SkillsInfo.GetValue<int>(skillName, "smokeDamage"),
                                    player,
                                    "weapon_smokegrenade"
                                );
        }
