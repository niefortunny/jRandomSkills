    ...
    public class JumpBan : ISkill
    {
        ...
        public static void PlayerJump(EventPlayerJump @event)
        {
            ...
            if (
                ...
                return;
            if (!bannedPlayers.TryGetValue(player.PlayerPawn.Value, out _))
                return;
            bannedPlayers.AddOrUpdate(
                player.PlayerPawn.Value,
                Server.TickCount + 10,
                (k, v) => Server.TickCount + 10
            );
        }
        ...
        public static void TypeSkill(CCSPlayerController player, string[] commands)
        {
            ...
            player.PrintToChat(
                $" {ChatColors.Green}"
                    + player.GetTranslation("jumpban_player_info", enemy.PlayerName)
            );
