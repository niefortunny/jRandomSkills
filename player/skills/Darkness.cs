...
namespace src.player.skills
{
    public class Darkness : ISkill
    {
        ...
        private static readonly ConcurrentDictionary<
            CCSPlayerController,
            List<CPostProcessingVolume>
        > defaultPostProcessings = [];
        ...
        public static void TypeSkill(CCSPlayerController player, string[] commands)
        {
            ...
            player.PrintToChat(
                $" {ChatColors.Green}"
                    + player.GetTranslation("darkness_player_info", enemy.PlayerName)
            );
        ...
        )
        {
            ...
            lock (setLock)
            {
                ...
                foreach (var postProcessingVolume in pawn.CameraServices.PostProcessingVolumes)
                {
                    ...
                    {
                        if (
                            defaultPostProcessings.TryGetValue(player, out var defaultList)
                            && i < defaultList.Count
                        )
                    ...
                    else
                    {
                        ...
                        var postProcessing = Utilities.CreateEntityByName<CPostProcessingVolume>(
