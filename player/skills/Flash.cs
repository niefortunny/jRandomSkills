...
namespace src.player.skills
{
    public class Flash : ISkill
    {
        ...
        public static void PlayerMakeSound(UserMessage um)
        {
            ...
            if (userIndex == 0)
                return;
            if (!Instance.footstepSoundEvents.Contains(soundevent))
                return;

            var player = Utilities
                .GetPlayers()
                .FirstOrDefault(p =>
                    p.Pawn?.Value != null && p.Pawn.Value.IsValid && p.Pawn.Value.Index == userIndex
                );
        ...
        public static void PlayerJump(EventPlayerJump @event)
        {
            ...
            if (player == null || !player.IsValid)
                return;
            if (!jumpedPlayers.TryGetValue(player.SteamID, out _))
                return;
            jumpedPlayers.AddOrUpdate(
                player.SteamID,
                Server.TickCount + 20,
                (k, v) => Server.TickCount + 20
            );
        }
        ...
        public static void EnableSkill(CCSPlayerController player)
        {
            ...
            SkillUtils.PrintToChat(
                player,
                $"{ChatColors.DarkRed}{player.GetSkillName(skillName)}{ChatColors.Lime}: {player.GetSkillDescription(skillName, newSpeed)}",
                false
            );
        }
        ...
        public static void OnTick()
        {
            foreach (var player in Utilities.GetPlayers())
            {
                ...
                if (
                    buttons.HasFlag(PlayerButtons.Moveleft)
                ...
                if (
                    jumpedPlayers.TryGetValue(player.SteamID, out var time)
                    && time > Server.TickCount
                )
                    continue;
