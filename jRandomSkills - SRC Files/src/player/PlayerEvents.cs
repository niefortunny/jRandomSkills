...
namespace src.player
{
    public static partial class Event
    {
        ...
        public static readonly SkillsInfo.DefaultSkillInfo[] terroristSkills =
        [
            .. SkillsInfo.LoadedConfig.Where(s => s.OnlyTeam == (int)CsTeam.Terrorist),
        ];
        public static readonly SkillsInfo.DefaultSkillInfo[] counterterroristSkills =
        [
            .. SkillsInfo.LoadedConfig.Where(s => s.OnlyTeam == (int)CsTeam.CounterTerrorist),
        ];
        private static readonly SkillsInfo.DefaultSkillInfo[] allTeamsSkills =
        [
            .. SkillsInfo.LoadedConfig.Where(s => s.OnlyTeam == 0),
        ];

        private static readonly ConcurrentDictionary<
            ulong,
            ConcurrentBag<jSkill_SkillInfo>
        > playersSkills = [];
        ...
        private static HookResult SmokegrenadeDetonate(
        ...
        private static HookResult SmokegrenadeExpired(
            EventSmokegrenadeExpired @event,
            GameEventInfo info
        )
        {
            lock (setLock)
        ...
        private static void OnTick()
        {
            lock (setLock)
            {
                foreach (var playerSkill in Instance.SkillPlayer)
                    if (!playerSkill.IsDrawing)
                        if (
                            SkillsInfo.GetValue<bool>(playerSkill.Skill, "disableOnFreezeTime")
        ...
        private static HookResult PlayerConnectFull(
        ...
        )
        {
            lock (setLock)
            {
                ...
                Instance.SkillPlayer.Add(
                    new jSkill_PlayerInfo
                    {
        ...
        private static HookResult RoundStart(EventRoundStart @event, GameEventInfo info)
        {
            lock (setLock)
            {
                ...
                freezeTimeEnd = DateTime.Now.AddSeconds(
                    freezetime + (Instance?.GameRules?.TeamIntroPeriod == true ? 7 : 0)
                );
                Instance?.AddTimer(
                    (Instance?.GameRules?.TeamIntroPeriod == true ? 7 : 0)
                        + Math.Max(freezetime - Config.LoadedConfig.SkillTimeBeforeStart, 0)
                        + .3f,
                    SetSkill
                );
                return HookResult.Continue;
                ...
                foreach (var player in Utilities.GetPlayers().Where(p => p.IsValid))
                {
                    Instance.AddTimer(
                        ...
                        () =>
                        {
                            ...
                            {
                                var _playerSkill = Instance.SkillPlayer.FirstOrDefault(p =>
                                    p.SteamID == _player.SteamID
                                );
                                if (_playerSkill != null)
                                {
                                    var skillInfo = SkillData.Skills.FirstOrDefault(p =>
                                        p.Skill == _playerSkill.Skill
                                    );
                                    var specialSkillInfo = SkillData.Skills.FirstOrDefault(s =>
                                        s.Skill == _playerSkill.SpecialSkill
                                    );
                                    if (skillInfo == null)
                                        continue;
                                    skillsText +=
                                        $" {ChatColors.DarkRed}{_player.PlayerName}{ChatColors.Lime}: {(_playerSkill.SpecialSkill == Skills.None || specialSkillInfo == null ? player.GetSkillName(skillInfo.Skill, _playerSkill.SkillChance) : $"{player.GetSkillName(specialSkillInfo.Skill)} -> {player.GetSkillName(skillInfo.Skill, _playerSkill.SkillChance)}")}\n";
                                }
                            }

                            if (
                                Config.LoadedConfig.SummaryAfterTheRound
                                && !string.IsNullOrEmpty(skillsText)
                            )
                            {
                                player.PrintToChat(" ");
                                player.PrintToChat(
                                    $" {ChatColors.Lime}{player.GetTranslation("summary_start")}"
                                );
                                foreach (string text in skillsText.Split("\n"))
                                    if (!string.IsNullOrEmpty(text))
                                        player.PrintToChat(text);
                                player.PrintToChat(
                                    $" {ChatColors.Lime}{player.GetTranslation("summary_end")}"
                                );
                                player.PrintToChat(" \n");
                            }
                        }
                ...
                if (Config.LoadedConfig.KillerSkillInfo)
                {
                    ...
                    if (attackerInfo != null)
                    {
                        var skillData = SkillData.Skills.FirstOrDefault(s =>
                            s.Skill == attackerInfo.Skill
                        );
                        var specialSkillData = SkillData.Skills.FirstOrDefault(s =>
                            s.Skill == attackerInfo.SpecialSkill
                        );
                        if (skillData == null || specialSkillData == null)
                            return HookResult.Continue;
                        string skillDesc = victim.GetSkillDescription(skillData.Skill);

                        SkillUtils.PrintToChat(
                            victim,
                            $"{ChatColors.Grey}{victim.GetTranslation("enemy_skill")} {ChatColors.White}{attacker.PlayerName}{ChatColors.Grey}:",
                            false
                        );
                        SkillUtils.PrintToChat(
                            victim,
                            $"{ChatColors.White}{skillDesc} {ChatColors.BlueGrey}[{ChatColors.Grey}{(attackerInfo.SpecialSkill == Skills.None ? victim.GetSkillName(skillData.Skill) : $"{victim.GetSkillName(specialSkillData.Skill)} -> {victim.GetSkillName(skillData.Skill)}")}{ChatColors.BlueGrey}]",
                            false
                        );
                    }
                }
        ...
        private static void CheckUseSkill(
        ...
        )
        {
            lock (setLock)
            {
                ...
                if (
                    SkillsInfo.GetValue<bool>(playerInfo.Skill, "disableOnFreezeTime")
                    && SkillUtils.IsFreezeTime()
                )
                    return;
                ...
                Debug.WriteToDebug(
                    $"Player {player.PlayerName} used the skill: {playerInfo.Skill} by PlayerButtons: {pressed}"
                );
        ...
        private static void SetSkill()
        {
            lock (setLock)
            {
                ...
                if (Config.LoadedConfig.GameMode == (int)Config.GameModes.TeamSkills)
                {
                    ...
                    tSkills.RemoveAll(s =>
                        s.Skill == tSkill.Skill
                        || s.Skill == Skills.None
                        || counterterroristSkills.Any(s2 => s2.Name == s.Skill.ToString())
                    );
                    tSkill =
                        tSkills.Count == 0
                            ? noneSkill
                            : tSkills[Instance.Random.Next(tSkills.Count)];

                    List<jSkill_SkillInfo> ctSkills = [.. SkillData.Skills];
                    ctSkills.RemoveAll(s =>
                        s.Skill == ctSkill.Skill
                        || s.Skill == Skills.None
                        || terroristSkills.Any(s2 => s2.Name == s.Skill.ToString())
                    );
                    ctSkill =
                        ctSkills.Count == 0
                            ? noneSkill
                            : ctSkills[Instance.Random.Next(ctSkills.Count)];
                }
                else if (Config.LoadedConfig.GameMode == (int)Config.GameModes.SameSkills)
                {
                    List<jSkill_SkillInfo> allSkills = [.. SkillData.Skills];
                    allSkills.RemoveAll(s =>
                        s.Skill == allSkill.Skill
                        || s.Skill == Skills.None
                        || !allTeamsSkills.Any(s2 => s2.Name == s.Skill.ToString())
                    );
                    allSkill =
                        allSkills.Count == 0
                            ? noneSkill
                            : allSkills[Instance.Random.Next(allSkills.Count)];
                }
                else if (
                    Config.LoadedConfig.GameMode == (int)Config.GameModes.Debug
                    && debugSkills.Count == 0
                )
                ...
                foreach (var player in validPlayers)
                {
                    ...
                    if (Instance?.GameRules != null && Instance?.GameRules.WarmupPeriod == false)
                    {
                        ...
                        if (staticSkills.TryGetValue(player.SteamID, out var staticSkill))
                            ...
                            gameMode == Config.GameModes.Normal
                            || gameMode == Config.GameModes.NoRepeat
                        )
                        {
                            List<jSkill_SkillInfo> skillList = [.. SkillData.Skills];
                            skillList.RemoveAll(s =>
                                s?.Skill == skillPlayer?.Skill
                                || s?.Skill == skillPlayer?.SpecialSkill
                                || s?.Skill == Skills.None
                            );

                            if (validPlayers.Count(p => p.Team == player.Team) == 1)
                            {
                                SkillsInfo.DefaultSkillInfo[] skillsNeedsTeammates =
                                [
                                    .. SkillsInfo.LoadedConfig.Where(s => s.NeedsTeammates),
                                ];
                                skillList.RemoveAll(s =>
                                    skillsNeedsTeammates.Any(s2 => s2.Name == s.Skill.ToString())
                                );
                            }
                            ...
                            if (
                                gameMode == Config.GameModes.NoRepeat
                                && playersSkills.TryGetValue(
                                    player.SteamID,
                                    out ConcurrentBag<jSkill_SkillInfo>? skills
                                )
                            )
                            {
                            ...
                            randomSkill =
                                skillList.Count == 0
                                    ? noneSkill
                                    : skillList[Instance.Random.Next(skillList.Count)];
                            if (gameMode == Config.GameModes.NoRepeat)
                            {
                                if (
                                    playersSkills.TryGetValue(
                                        player.SteamID,
                                        out ConcurrentBag<jSkill_SkillInfo>? value
                                    )
                                )
                        ...
                        else if (gameMode == Config.GameModes.Debug)
                        {
                            ...
                            player.PrintToChat(
                                $"{SkillData.Skills.Count - debugSkills.Count}/{SkillData.Skills.Count}"
                            );
                        }
                    }
                    ...
                    if (randomSkill.Display)
                        SkillUtils.PrintToChat(
                            player,
                            $"{player.GetSkillDescription(randomSkill.Skill)} {ChatColors.BlueGrey}[{ChatColors.Grey}{player.GetSkillName(randomSkill.Skill)}{ChatColors.BlueGrey}]",
                            false
                        );
                    ...
                    if (
                        SkillsInfo.GetValue<bool>(randomSkill.Skill, "disableOnFreezeTime")
                        && SkillUtils.IsFreezeTime()
                    )
                        Instance?.AddTimer(
                            Config.LoadedConfig.SkillTimeBeforeStart,
                            () =>
                            {
                                if (
                                    Instance.SkillPlayer.FirstOrDefault(p =>
                                        p.SteamID == player.SteamID && p.Skill == randomSkill.Skill
                                    ) == null
                                )
                                    return;
                                Instance?.SkillAction(
                                    randomSkill.Skill.ToString(),
                                    "EnableSkill",
                                    [player]
                                );
                            }
                    ...
                    Debug.WriteToDebug(
                        $"Player {skillPlayer.PlayerName} has got the skill \"{player.GetSkillName(randomSkill.Skill)}\"."
                    );
                    ...
                    if (Config.LoadedConfig.TeamMateSkillInfo)
                    {
                        Instance?.AddTimer(
                            ...
                            () =>
                            {
                                ...
                                {
                                    var teammateInfo = Instance.SkillPlayer.FirstOrDefault(p =>
                                        p.SteamID == teammate.SteamID
                                    );
                                    if (teammateInfo != null && teammateInfo?.Skill != null)
                                    {
                                        var skillInfo = SkillData.Skills.FirstOrDefault(p =>
                                            p.Skill == teammateInfo.Skill
                                        );
                                        teammateSkills +=
                                            $" {ChatColors.DarkRed}{teammate.PlayerName}{ChatColors.Lime}: {(skillInfo == null ? player.GetSkillName(Skills.None) : player.GetSkillName(skillInfo.Skill, teammateInfo.SkillChance))}\n";
                                    }
                                }
                                ...
                                if (!string.IsNullOrEmpty(teammateSkills))
                                {
                                    SkillUtils.PrintToChat(
                                        player,
                                        $" {ChatColors.Lime}{player.GetTranslation("teammate_skills")}:",
                                        false
                                    );
        ...
        public static void SetRandomSkill(CCSPlayerController player)
        {
            lock (setLock)
            {
                ...
                if (randomSkill.Display)
                    SkillUtils.PrintToChat(
                        player,
                        $"{ChatColors.Grey}{player.GetSkillDescription(randomSkill.Skill)} [{ChatColors.White}{player.GetSkillName(randomSkill.Skill)}{ChatColors.Grey}]",
                        false
                    );
        ...
        )
        {
            lock (setLock)
            {
                ...
                var emptySymbol =
                    $"<font class='fontSize-{(string.IsNullOrEmpty(headerLine) ? "l" : "ml")}'> </font>";
                ...
                string skillLine =
                    $"{emptySymbol2}<font class='fontWeight-Bold fontSize-{config.SkillLineSize}'>{centerLine}</font>{emptySymbol2}";
