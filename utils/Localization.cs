...
namespace src.utils
{
    ...
    {
        private static readonly string languagesFolderPath = Path.Combine(
            jRandomSkills.Instance.ModuleDirectory,
            "languages"
        );
        ...
        private static readonly string configsFolderPath = Path.Combine(
            jRandomSkills.Instance.ModuleDirectory,
            "configs"
        );
        private static ConcurrentDictionary<ulong, string> _playersLanguage = [];
        private static readonly string geoliteFilePath = Path.Combine(
            jRandomSkills.Instance.ModuleDirectory,
            "packages",
        ...
        private static void LoadLanguage(string langPath)
        {
            ...
            var translations = JsonConvert.DeserializeObject<ConcurrentDictionary<string, string>>(
                jsonText
            );
        ...
        )
        {
            ...
            if (skillNameText.Contains('%'))
                skillNameText = skillNameText.Replace(
                    value.ToString(),
                    Math.Round(value * 100, 0).ToString()
                );
        ...
        )
        {
            ...
            var skilLDescription =
                desc2 == skillName ? player.GetTranslation($"{skill.ToString().ToLower()}_desc")
                : desc2.Contains('%')
                    ? desc2.Replace(value.ToString(), Math.Round(value * 100, 0).ToString())
                : desc2;
            return skilLDescription;
        }

        public static void PrintTranslationToChatAll(
            string message,
            string[]? key,
            params object[][]? args
        )
        ...
        )
        {
            ...
            if (
                _translations.TryGetValue(langCode, out var langDict)
        ...
        private static string GetLangCode(CCSPlayerController? player)
        {
            ...
            string? geoliteLandCode =
                GetLangCodeFromDatabase(GetPlayerIP(player)) ?? defaultLangCode;
        ...
        private static string? GetLangCodeFromDatabase(string? playerIP)
        {
            if (string.IsNullOrEmpty(playerIP))
                return null;
            if (!File.Exists(geoliteFilePath))
                return null;
            ...
            if (
                data.TryGetValue("country", out var _country)
                && _country is ConcurrentDictionary<string, object> country
            )
        ...
        private static void LoadPlayersLanguage()
        {
            ...
            _playersLanguage =
                JsonConvert.DeserializeObject<ConcurrentDictionary<ulong, string>>(jsonText) ?? [];
        }
